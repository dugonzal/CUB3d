...
/*   Updated: 2023/07/19 13:12:07 by masla-la         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int get_rgb_tmp(t_game *game, int i, char *line)
{
	game->color[i].buffer = ft_strtrim(&line[1], " \t\r\v\f\n", 0);
	if (ft_strlen(game->color[0].buffer) < 5 || check_commas(game, 0))
	  return (1);
	game->color[i].rgb = ft_split(game->color[i].buffer, ',', 0);
	free(game->color[i].buffer);
	if (get_rgb(game, i))
	  return (1);
	free_array(game->color[0].rgb);
	return (0);
}
...
int _get(t_game *game, char *line, int iter)
{
  ...
  {
	if (get_rgb_tmp(game, 0, line))
	  return (1);
  }
  else if (line[0] == 'C' && iter == 5)
  {
	game->color[1].buffer = ft_strtrim(&line[1], " \t\r\v\f\n", 0);
	if (ft_strlen(game->color[1].buffer) < 5 || check_commas(game, 1))
	  return (1);
	game->color[1].rgb = ft_split(game->color[1].buffer, ',', 0);
	free(game->color[1].buffer);
	if (get_rgb(game, 1))
	  return (1);
	free_array(game->color[1].rgb);
	printf("color: %d %d %d\n", game->color[1].r, game->color[1].g, game->color[1].b);
  }
  else
	  return (1);
  return (0);
